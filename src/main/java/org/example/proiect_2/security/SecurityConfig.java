package org.example.proiect_2.security;
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
        http
                .authorizeHttpRequests(authConfig -> {
                    authConfig.requestMatchers(HttpMethod.GET, "/", "/registration/login","/admin/**","/user/**", "/registration/register", "/registration/error", "/registration/login-error", "/logout","/registration/createUser","/registration/deleteUser","/css/**").permitAll();
                    authConfig.requestMatchers(HttpMethod.POST, "/create","/registration/createUser","/deleteUser","/createCar","/admin/**").permitAll();
                    ...
                    authConfig.requestMatchers(HttpMethod.GET, "/admin/**").hasAuthority("ADMIN");
                    authConfig.requestMatchers(HttpMethod.POST, "/admin/**").hasAuthority("ADMIN");
                    authConfig.requestMatchers(HttpMethod.GET, "/user/**").hasAuthority("USER");
                    authConfig.requestMatchers(HttpMethod.POST, "/user/**").hasAuthority("USER");
                ...
                .formLogin(login -> {
                            login.loginPage("/registration/login");
                            login.successHandler((request, response, authentication) -> {
                                if (authentication.getAuthorities().stream().anyMatch(grantedAuthority -> grantedAuthority.getAuthority().equals("ADMIN"))) {
                                    response.sendRedirect("/admin/users/read");
                                } else if (authentication.getAuthorities().stream().anyMatch(grantedAuthority -> grantedAuthority.getAuthority().equals("USER"))) {
                                    response.sendRedirect("/user/cars/read");
                                } else {
